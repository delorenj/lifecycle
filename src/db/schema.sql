-- Lifecycle Controller Database Schema
-- Single source of truth for lifecycle state, history, observations, and outbox.
--
-- Design principles:
-- - lifecycle_state = materialized current truth (fast reads)
-- - lifecycle_status_history = append-only audit log (compliance/debugging)
-- - lifecycle_observations = raw facts from sentinels (TTL'd, replaceable)
-- - lifecycle_reconcile_queue = dirty-queue + lease pattern (event-driven + sweeper)
-- - lifecycle_event_outbox = transactional outbox (exactly-once publishing)
-- - sentinel_heartbeats = observer liveness tracking
...
CREATE TABLE IF NOT EXISTS lifecycles (
    ...
    name            TEXT NOT NULL,
    repo            TEXT NOT NULL,
    repos           TEXT, -- JSON array for multi-repo lifecycles
    roadmap_id      TEXT,
...
-- ---------------------------------------------------------------------------
-- Materialized current state (separate from registry for lock-free reads)
-- ---------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS lifecycle_state (
    ...
    state_fingerprint   TEXT, -- hash of deterministic state for quick diff
    updated_at          TIMESTAMPTZ DEFAULT now(),
    policy              JSONB DEFAULT '{}'
);
...
-- ---------------------------------------------------------------------------
-- Status history (append-only, partitioned by time if scale demands)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Blockers (current active blockers, deduplicated by kind+scope)
-- ---------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS lifecycle_blockers (
    ...
    fingerprint     TEXT -- stable hash for dedup
);
...
-- ---------------------------------------------------------------------------
-- Gates (intentional pauses/review points)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Checkpoints (roadmap milestone tracking)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Observations (raw facts from sentinels — TTL'd, aggressively pruned)
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS lifecycle_observations (
    id              BIGSERIAL PRIMARY KEY,
    lifecycle_id    TEXT NOT NULL REFERENCES lifecycles(id) ON DELETE CASCADE,
    source          TEXT NOT NULL, -- e.g. 'plane-sentinel', 'git-sentinel'
    kind            TEXT NOT NULL, -- e.g. 'work_items_snapshot', 'repo_activity_snapshot'
...
-- ---------------------------------------------------------------------------
-- Reconcile queue (dirty-queue + lease pattern)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Event outbox (transactional outbox for exactly-once publishing)
-- ---------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS lifecycle_event_outbox (
    ...
    payload         JSONB NOT NULL,
    headers         JSONB, -- correlationid, causationid, etc.
    created_at      TIMESTAMPTZ DEFAULT now(),
...
-- ---------------------------------------------------------------------------
-- Sentinel heartbeats (observer liveness)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Helper: prune expired observations (call from sweeper or cron)
-- ---------------------------------------------------------------------------

-- DELETE FROM lifecycle_observations WHERE expires_at < now();
